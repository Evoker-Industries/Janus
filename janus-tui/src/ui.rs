...
fn draw_status(f: &mut Frame, app: &App, area: Rect) {
    ...
    if let Some(ref status) = app.status {
        lines.extend(vec![
            ...
            Line::from(vec![
                Span::raw("Uptime: "),
                Span::styled(format_duration(status.uptime_secs), Style::default().fg(Color::Green)),
            ]),
            Line::from(vec![
                Span::raw("Active Connections: "),
                Span::styled(status.active_connections.to_string(), Style::default().fg(Color::Magenta)),
            ]),
            Line::from(vec![
                Span::raw("Routes: "),
                Span::styled(status.route_count.to_string(), Style::default().fg(Color::Blue)),
            ]),
            Line::from(vec![
                Span::raw("Upstreams: "),
                Span::styled(status.upstream_count.to_string(), Style::default().fg(Color::Blue)),
            ]),
        ]);
    } else {
        lines.push(Line::styled("No status data available", Style::default().fg(Color::DarkGray)));
    }
...
fn draw_routes(f: &mut Frame, app: &App, area: Rect) {
    let header_cells = ["Path", "Methods", "Upstream", "Timeout"]
        .iter()
...
fn draw_upstreams(f: &mut Frame, app: &App, area: Rect) {
            ...
            .map(|(i, (name, upstream))| {
                ...
                ListItem::new(vec![
                    ...
                    Line::from(vec![
                        ...
                        Span::styled(format!("{:?}", upstream.load_balancing), Style::default().fg(Color::Yellow)),
                    ]),
                ])
                .style(style)
            })
    ...
    let list = List::new(items)
        .block(Block::default().borders(Borders::ALL).title("Upstreams (j/k to navigate, d to delete)"));
    ...
    if let Some(ref stats) = app.stats {
        lines.extend(vec![
            Line::from(vec![
                ...
                Span::styled(stats.total_requests.to_string(), Style::default().fg(Color::Green)),
            ]),
            Line::from(vec![
                Span::raw("Requests/sec: "),
                Span::styled(format!("{:.2}", stats.requests_per_second), Style::default().fg(Color::Yellow)),
            ]),
            Line::from(vec![
                Span::raw("Bytes Received: "),
                Span::styled(format_bytes(stats.bytes_received), Style::default().fg(Color::Cyan)),
            ]),
            Line::from(vec![
                Span::raw("Bytes Sent: "),
                Span::styled(format_bytes(stats.bytes_sent), Style::default().fg(Color::Cyan)),
            ]),
            Line::raw(""),
            Line::styled("Status Codes:", Style::default().add_modifier(Modifier::BOLD)),
            Line::from(vec![
                Span::raw("  2xx (Success): "),
                Span::styled(stats.status_codes.success.to_string(), Style::default().fg(Color::Green)),
            ]),
            Line::from(vec![
                Span::raw("  3xx (Redirect): "),
                Span::styled(stats.status_codes.redirect.to_string(), Style::default().fg(Color::Blue)),
            ]),
            Line::from(vec![
                Span::raw("  4xx (Client Error): "),
                Span::styled(stats.status_codes.client_error.to_string(), Style::default().fg(Color::Yellow)),
            ]),
            Line::from(vec![
                Span::raw("  5xx (Server Error): "),
                Span::styled(stats.status_codes.server_error.to_string(), Style::default().fg(Color::Red)),
            ]),
        ]);
    } else {
        lines.push(Line::styled("No statistics available", Style::default().fg(Color::DarkGray)));
    }
